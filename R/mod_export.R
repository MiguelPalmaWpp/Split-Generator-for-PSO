...
mod_export_ui <- function(id) {
  ...
  tagList(
    ...
    # ── Preview panel (shown when a preview button is clicked) ─
...
mod_export_server <- function(id, data, results, config) {
  moduleServer(id, function(input, output, session) {
    ...
    output$export_status <- renderUI({
      res_list  <- results()
      d         <- data()
      ch_names  <- names(d$channels_meta %||%
                           tryCatch(config()$channel_names,
                                    error = \(e) NULL) %||%
                           names(res_list))
      n_done    <- length(res_list)
      n_total   <- max(n_done, 1L)
      
      # Build per-channel status rows
      channel_rows <- if (n_done > 0) {
        lapply(names(res_list), function(nm) {
          ...
          div(
            ...
            tags$span(tags$strong(nm),
                      style = "flex:1; color:#2c3e50;"),
            tags$span(paste0(n_splits, " splits"),
                      style = paste0(
      ...
      # Warning if results seem incomplete
    ...
    # ── Core reactives (existing logic, compatible with reactiveVal) ──
    ...
    final_analytical <- reactive({
      ...
      for (r in res_list) {
        if (!is.null(r$cross_cols)) {
          cross_cols_used <- r$cross_cols; break
        }
      }
      ...
      # Convert analytical to data.table 
      ...
      for (r in res_list) {
        ...
        # Fill NAs only in the NEW split columns just added — not the full df
        new_cols     <- setdiff(names(rag_dt), join_cols)
        num_new_cols <- new_cols[
          sapply(result_dt[, .SD, .SDcols = new_cols], is.numeric)
        ]
        for (col in num_new_cols) {
          data.table::set(result_dt, which(is.na(result_dt[[col]])), col, 0L)
        }
        
        gc()   # release memory after each channel join
      }
    ...
    final_side_mapping <- reactive({
      ...
      map(res_list, \(r) r$side_mapping) %>%
        ...
        mutate(
          ...
          .srt = if_else(str_detect(VariableSplit, "_Before"),
                         1L, 2L)
        ) %>%
