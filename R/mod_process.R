    ...
    tagList(
      card(
        ...
        downloadButton(ns("dl_config_process"),
                       label = tagList(icon("download"), " Download config JSON"),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    apply_single_merge <- function(res, merge_entry, cfg) {
      ...
      if (nrow(selected_diag) == 0) {
        showNotification(paste0("Saved merge '", new_name,
                                "': no data for view '", view_filter, "' — skipped."),
                         type = "warning", duration = 6)
        return(res)
      }
    ...
    cross_section_choices <- reactive({
      ...
      cross_keys   <- do.call(paste, c(as.list(cross_data), list(sep = " / ")))
      sort(unique(cross_keys))
    })
    
    observeEvent(input$channel_select, { cs_selected("ALL") }, ignoreInit = TRUE)
    ...
    output$dl_config_process <- downloadHandler(
      ...
      content = function(file) {
        ...
        jsonlite::write_json(cfg_data, path = file, pretty = TRUE,
    ...
    run_one <- function(nm) {
      ...
      if (length(missing_cc) > 0) {
        showNotification(
          paste0(nm, ": cross-section column(s) '",
                 ...
                 "' not found in AnalyticalDataset. Check Config tab."),
      ...
      withProgress(message = paste("Processing", nm, "..."), value = 0.5, {
        tryCatch({
    ...
    output$period_filter_ui <- renderUI({
      ...
      tagList(
        ...
        div(style = "display:flex; gap:8px; font-size:12px;",
            tags$span(style = "background:#EBF3FB; color:#5B9BD5; padding:2px 8px; border-radius:10px; font-weight:600;",
                      paste0("Focus: ", n_focus)),
            tags$span(style = "background:#f4f6f9; color:#6c757d; padding:2px 8px; border-radius:10px; font-weight:600; border:1px solid #dee2e6;",
                      paste0("Non-Focus: ", n_nf)),
            tags$span(style = "background:#fff3cd; color:#856404; padding:2px 8px; border-radius:10px; font-weight:600;",
                      paste0("Total: ", n_all))
        ))
      )
    })
    ...
    current_act_data <- reactive({
      ...
      if (isTRUE(res$is_rag)) {
        ...
        if (filter_val == "focus")    df <- df[df$period == "focus", ]
        if (filter_val == "nonfocus") df <- df[df$period == "nonfocus", ]
        df %>% filter(total_activity > 0) %>%
          select(-any_of(c("seg", "period", "model_var"))) %>%
          mutate(across(where(is.numeric), \(x) round(x, 4)))
        
      } else {
        cfg          <- channels()[[nm]]; gcfg <- config()
        cross_cols_r <- res$cross_cols %||% "Geography"
        rag_df       <- as.data.frame(res$rag)
        ...
        if (selected_cs == "ALL") {
          ...
          rag_df <- aggregate(rag_df[, num_cols_r, drop = FALSE],
        ...
        id_cols <- intersect(
          if (selected_cs == "ALL") "Period" else c(cross_cols_r, "Period"),
          names(rag_df)
        )
        
        act_cols_keyword <- grep(act_kw, names(rag_df), ignore.case = TRUE, value = TRUE)
        act_cols_merged  <- intersect(
          unique(res$act_diagnoses$VariableSplit),  
    ...
    output$activity_kpis <- renderUI({
      ...
      kpis <- list(
        ...
        list(label = paste0("Above ", threshold, "%"), value = above_thresh,
             icon = "arrow-up",   color = "#2ecc71",  bg = "#e8f5e9"),
        list(label = paste0("Below ", threshold, "% (review)"), value = below_thresh,
             icon = "arrow-down", color = "#e74c3c",  bg = "#fdecea"),
      ...
      do.call(layout_columns, c(
        ...
        lapply(kpis, function(k) {
          div(style = paste0(
            "background:", k$bg, "; border-left:3px solid ", k$color, ";",
    ...
    output$threshold_ui <- renderUI({
      ...
      tags$span(style = "font-size:11px; color:#adb5bd;",
                icon("circle-info"), " Splits below threshold shown in red.",
                tags$br(), " Select manually based on grouping logic.")
      )
    })
    
    # ── Merge Plan toolbar: Download CSV + Upload filled plan ──────
    output$merge_plan_toolbar <- renderUI({
      ...
      div(
        ...
        # Download merge plan CSV
        downloadButton(
          session$ns("dl_merge_plan"),
          label = tagList(icon("download"), " Download Merge Plan"),
          class = "btn-outline-secondary btn-sm"
        ),
        
        # Upload filled merge plan
        div(
        ...
        tags$span(
          ...
          " Fill MergeGroup (1,1,2,2...) and MergeName, then upload"
        )
      )
    })
    ...
    output$dl_merge_plan <- downloadHandler(
      ...
      content = function(file) {
        ...
        df <- tryCatch(
          isolate(current_act_data()),
          error = function(e) {
            message("dl_merge_plan error: ", conditionMessage(e))
            NULL
          }
        )
        ...
        if (is.null(df) || nrow(df) == 0) {
          write.csv(
            data.frame(VariableSplit = character(), Split = character(),
                       MergeGroup   = character(), MergeName = character()),
        ...
        df_download <- df %>%
          mutate(
            Split      = strip_common_prefix(VariableSplit),
            MergeGroup = NA_character_,   
    ...
    # ── Apply Merge Plan from uploaded CSV ─────────────────────────
    observeEvent(input$merge_plan_content, {
      ...
      missing_cols <- setdiff(c("VariableSplit", "MergeGroup", "MergeName"),
                              names(plan))
      ...
      plan_active <- plan[
        !is.na(plan$MergeGroup) &
          nzchar(trimws(as.character(plan$MergeGroup))), ]
      
      if (nrow(plan_active) == 0) {
        showNotification(
          "No merge groups found. Fill MergeGroup with numbers (1,1,2,2...).",
          type = "warning"); return()
      }
      ...
      groups      <- split(plan_active,
                           as.character(plan_active$MergeGroup))
      ...
      withProgress(message = "Applying merge plan...", value = 0, {
        for (i in seq_along(groups)) {
          grp    <- groups[[i]]
          grp_id <- names(groups)[i]
          incProgress(1 / length(groups))
          
          mn_vals    <- trimws(as.character(grp$MergeName))
          merge_name <- mn_vals[!is.na(mn_vals) & nzchar(mn_vals)]
          merge_name <- if (length(merge_name)) merge_name[1] else grp_id
          ...
          if (!length(rag_cols)) {
            ...
            showNotification(
              paste0("Group ", grp_id, " (", merge_name,
                     "): splits not in RAG — skipped."),
          ...
          if (nrow(selected_diag) == 0) {
            n_skipped <- n_skipped + 1L
            showNotification(
              paste0("Group ", grp_id, ": no data for '",
                     view_filter, "' — skipped."),
              type = "warning", duration = 5); next
          }
          ...
          # 1. RAG
          new_rag <- res$rag
          new_rag[[merge_name]] <- rowSums(new_rag[, rag_cols, drop = FALSE],
                                           na.rm = TRUE)
          new_rag <- new_rag[, setdiff(names(new_rag), rag_cols)]
          
          # 2. act_diagnoses
          merged_act <- tibble(
            VariableSplit         = merge_name,
            total_activity        = sum(selected_diag$total_activity,        na.rm = TRUE),
            pct_total_activity    = NA_real_,
            num_weeks_activity    = max(selected_diag$num_weeks_activity,    na.rm = TRUE),
            max_index             = NA_real_,
          ...
          new_act_diag <- res$act_diagnoses %>%
            ...
            group_by(period) %>%
            mutate(grand_p = sum(total_activity, na.rm = TRUE),
                   pct_total_activity = round(total_activity /
                                                pmax(grand_p, 1) * 100, 4)) %>%
            ungroup() %>% select(-grand_p)
          
          # 3. activity_spend
          merged_as <- res$activity_spend %>%
            filter(VariableSplit %in% selected_splits) %>%
            summarise(VariableSplit = merge_name,
                      total_activity = sum(total_activity, na.rm = TRUE),
                      total_spend    = sum(total_spend,    na.rm = TRUE),
                      Channel = first(Channel),
                      MainModelVariableName = first(MainModelVariableName))
          ...
          merged_sm <- res$side_mapping %>%
            filter(VariableSplit %in% selected_splits) %>%
            slice(1) %>% mutate(VariableSplit = merge_name)
          new_side_map <- res$side_mapping %>%
            filter(!VariableSplit %in% selected_splits) %>%
            bind_rows(merged_sm)
          
          # 5. cost_diagnoses mirror
          ...
          new_cost_diag <- tryCatch({
            if (!length(matching_cost)) {
              ...
              if ("period" %in% names(cc)) {
                cc %>% group_by(period) %>%
                  mutate(gp = sum(total_spend, na.rm = TRUE),
                         pct_total_spend = round(total_spend /
                                                   pmax(gp, 1) * 100, 4)) %>%
                  ungroup() %>% select(-gp)
              } else {
                gt <- sum(cc$total_spend, na.rm = TRUE)
                cc %>% mutate(pct_total_spend = round(total_spend /
                                                        pmax(gt, 1) * 100, 4))
              }
            }
          }, error = \(e) res$cost_diagnoses)
          ...
          res <- list(
            rag = new_rag, cross_cols = res$cross_cols,
            is_rag = res$is_rag, ref_cross = res$ref_cross,
            activity_spend = new_act_spend, side_mapping = new_side_map,
            act_diagnoses = new_act_diag, cost_diagnoses = new_cost_diag
          )
      ...
      set_res(nm, res)
      set_log(nm, c(get_log(nm), new_log))
      ...
      n_ok <- length(new_log)
      showNotification(
        paste0(n_ok, " group(s) merged from plan",
    ...
    output$merge_toolbar <- renderUI({
      ...
      hint <- if (nchar(parts$prefix) > 0 || nchar(parts$suffix) > 0)
        div(style = "font-size:11px; color:#8a9bb0; margin-top:4px;",
            icon("circle-info", style = "font-size:10px;"),
            " Detected pattern: ",
            tags$code(style = "background:#EBF3FB; color:#5B9BD5; padding:1px 5px; border-radius:3px; font-size:11px;",
    ...
    observeEvent(input$btn_merge, {
      ...
      if (!nchar(new_name)) {
        showNotification("Enter a name for the merged split.", type = "warning")
        return()
      }
      ...
      if (!length(rag_cols)) {
        showNotification("Selected splits not found in RAG.", type = "warning")
        return()
      }
      ...
      if (nrow(selected_diag) == 0) {
        showNotification(
          paste0("No diagnosis data for '", view_filter,
    ...
    observeEvent(input$btn_save_merges, {
      ...
      new_saved <- lapply(seq_along(current_log), function(i) {
        c(current_log[[i]], list(id       = max_id + i,
                                 active   = TRUE,
                                 saved_at = format(Sys.time(), "%Y-%m-%d %H:%M")))
      })
      update_merges(nm, c(existing, new_saved))
    ...
    # ── Toggle saved merge active/inactive ────────────────────────
    observe({
      results_trigger()
            ...
            lapply(rev(seq_along(log)), function(i) {
              ...
              div(class = "mb-2 p-2",
                  ...
                  div(class = "d-flex align-items-center gap-2 mb-1",
                      ...
                      if (i == length(log))
                        tags$span("latest",
                                  style = "font-size:9px; color:#8a9bb0; margin-left:auto;")),
    ...
    observeEvent(input$btn_reset_merges, {
      ...
      showModal(modalDialog(
        ...
        tags$p("This will reset all merges for channel ",
               tags$strong(nm), "."),
        if (n_merges > 0)
          tags$p(class = "text-muted small",
                 paste0(n_merges, " session merge(s) will be lost.")),
        footer = tagList(
          actionButton(session$ns("btn_confirm_reset"),
                       "Reset all merges",
                       class = "btn-danger"),
          modalButton("Cancel")
        ),
    ...
    # ── Activity table (no DT Buttons — download via Merge Plan) ──
    output$diag_act <- DT::renderDT({
      ...
      num_fmt <- intersect(c("sd", "min", "quartile_1", "median", "quartile_3",
                             "max_no_outlier", "max"), names(df_display))
    ...
    output$diag_check <- DT::renderDT({
      ...
      if (length(valid_vars) == 0) {
        return(datatable(
          data.frame(Message = paste(
            "Model variables not found in AnalyticalDataset:",
            paste(as.character(cfg$model_variables %||% "none"),
                  collapse = ", ")
          )),
      ...
      model_side <- build_model_total(
        d$analytical, cross_id, cfg$model_variables, cfg$break_dates
      )
      
      rag_df      <- as.data.frame(res$rag)
      splits_side <- rag_df %>%
        select(all_of(cross_id), where(is.numeric)) %>%
        mutate(SplitsTotal = rowSums(select(., where(is.numeric)), na.rm = TRUE)) %>%
