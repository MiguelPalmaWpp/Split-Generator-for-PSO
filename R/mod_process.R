  ...
  moduleServer(id, function(input, output, session) {
    ...
    run_one <- function(nm) {
      ...
      if (length(cfg$break_dates) != n - 1) {
        showNotification(paste0(nm, ": expected ", n - 1, " break date(s), got ",
      ...
      if (!is.null(res_stored)) {
        ...
        if (length(active_saved) > 0) {
          pending$nm <- nm; pending$base_res <- res_stored
          ...
          showModal(modalDialog(
            ...
            footer = tagList(
              actionButton(session$ns("btn_modal_apply"),
                           ...
                           class = "btn-primary"),
              actionButton(session$ns("btn_modal_skip"), "Process without merges",
    ...
    # ── Period filter UI — Focus vs Non-Focus only, no cross-section
    ...
    current_act_data <- reactive({
      ...
      if (!isTRUE(res$is_rag)) {
        ...
        # Always ALL — sum all cross-sections
    ...
    output$activity_kpis <- renderUI({
      ...
      do.call(layout_columns, c(
        ...
        lapply(kpis, function(k) {
          div(style = paste0("background:", k$bg,
    ...
    # ── Merge plan toolbar — updated hint text ─────────────────────
    ...
    # ── Download Merge Plan — single MergeName column ─────────────
    ...
    # ── Apply Merge Plan — groups by MergeName directly ───────────
    observeEvent(input$merge_plan_content, {
      ...
      # Only need VariableSplit + MergeName
      ...
      # Rows with a MergeName = rows to process
      plan_active <- plan[
        !is.na(plan$MergeName) & nzchar(trimws(as.character(plan$MergeName))), ]
      
      if (nrow(plan_active) == 0) {
        showNotification(
          "No merge names found. Write the same MergeName on splits you want to merge.",
          type = "warning"); return()
      }
      ...
      # Group by MergeName — same name = same merge group
    ...
    # ── Activity table — no pagination, all splits visible ─────────
    ...
    output$diag_check <- DT::renderDT({
      ...
      geo_map <- tibble(an_geo = an_geos, norm = normalize_geo(an_geos)) %>%
        left_join(
          tibble(rag_geo = rag_geos, norm = normalize_geo(rag_geos)),
          by = "norm"
        ) %>%
